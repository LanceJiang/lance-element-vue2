<template>
  <div
    ...
    <transition
      ...
      <component
        :is="curComponent"
        :class="['el-select-dropdown', multiple ? 'is-multiple' : '' ]"
        ref="popper"
...
export default {
  ...
  computed: {
    ...
    // 页面展示数据（options是所有数据）
    computedOptions() {
      return this.options.filter(v => v.visible)
    },
    wrapStyle() {
  ...
  watch: {
    ...
    value(val, oldVal) {
      const isPopover = this.isPopover
      if (this.multiple) {
        // if ((this.$refs.input && this.query !== '')) {
        //   this.currentPlaceholder = ''
        // } else if (val && val.length > 0) {
        //   this.currentPlaceholder = isPopover ? '' : this.cachedPlaceHolder
        if ((val && val.length > 0) || (this.$refs.input && this.query !== '')) {
          this.currentPlaceholder = ''
          // let placeholder = ''
          // if (isPopover && this.query) {
          //   placeholder = this.cachedPlaceHolder
          // }
    ...
    options() {
      ...
      // this.computedOptions = this.options.filter(v => v.visible)
      // this.updateComputedOptions()
    }

    // filteredOptionsCount(num) {
    //   console.error('filteredOptionsCount.....', num)
    //   // this.updateComputedOptions()
    // }
  },
  ...
  methods: {
    ...
    setSelected() {
      if (!this.multiple) {
        ...
        this.$emit('update:selected_label', option.currentLabel)
      ...
      this.$emit('update:selected_label', result.map(v => v.currentLabel))
      this.$nextTick(() => {
        this.resetInputHeight()
      })
    },
    ...
    handleOptionSelect(option, byClick) {
      // console.error(option, 'option byClick', byClick)
      if (this.multiple) {
        const value = (this.value || []).slice()
    ...
    checkAllHandler() {
      // cachedOptions
      const localValues = this.computedOptions.map(v => v.value)
      let value = []
      if (this.isAll) {
        // 取消全选
        value = this.value.filter(v => !localValues.includes(v))
      } else {
        // 全选
        value = [...new Set((this.value || []).concat(localValues))]
      }
      this.$emit('input', value)
      this.emitChange(value)
    }
    ...
    //   if (this.multiple) {
    //     // this.computedOptions = this.options.filter(v => v.visible)
    //     console.log(this.computedOptions, 'this.computedOptions', this.computedOptions.map(v => {
    //       return {
    //         value: v.value,
    //         visible: v.visible
    //       }
  ...
  mounted() {
    if (this.multiple && Array.isArray(this.value) && this.value.length > 0 && this.isPopover) {
      this.currentPlaceholder = ''
    }
